...
int main(void)
{
    ...
    float appleSpawnInterval = 5.0f;
    float appleSpawnTimer = 0.0f;

    while(!WindowShouldClose())
    {
        if(IsKeyPressed(KEY_P) && gamePaused)
        {
            gamePaused = false;
        }
        else if(IsKeyPressed(KEY_P) && !gamePaused)
        {
            gamePaused = true;
        }

        if(!gameInProgress && IsKeyPressed(KEY_R))
        {
            snakeLength = 4;
            for(int i = 0; i < snakeLength; i++)
            {
        ...
        if(!gameInProgress)
        {
            ...
            int posY = floor(screenHeight / 2) - fontSize;
        ...
        else if (gamePaused)
        {
            ...
            int fontSize = 40;
            int textLength = MeasureText(gamePausedText, fontSize);

            int posX = floor(screenWidth / 2) - floor(textLength / 2);
            int posY = floor(screenHeight / 2) - fontSize;
            DrawText(gamePausedText, posX, posY, fontSize, BLACK);
